...
def get_system_stats():
    ...
    prevention_rate = (
        (blocked / total_submissions * 100) if total_submissions > 0 else 0
    )
        ...
        jsonify(
            {
                ...
                "low_quality_queue": {
                    ...
                    "total": low_quality_pending
                    + low_quality_approved
...
def get_duplicate_stats():
    ...
    top_parents = (
        ...
        .join(Bug, Bug.parent_bug_id == Bug.id, isouter=True)
        .group_by(Bug.id)
        .having(func.count(Bug.id) > 0)
    ...
    # Recent duplicates
    recent = (
        DuplicateHistory.query.order_by(DuplicateHistory.detected_at.desc())
        .limit(20)
        .all()
    )
    ...
    by_method = (
        ...
        .group_by(DuplicateHistory.match_method)
        .all()
    )
        ...
        jsonify(
            {
                ...
                "by_detection_method": {method: count for method, count in by_method},
            }
        ),
        200,
    )
...
def get_region_stats():
    ...
    bugs_by_region = (
        db.session.query(Bug.region, func.count(Bug.id).label("count"))
        .group_by(Bug.region)
        .all()
    )

    duplicates_by_region = (
        db.session.query(Bug.region, func.count(Bug.id).label("count"))
        .filter(Bug.classification_tag == "Duplicate")
        .group_by(Bug.region)
        .all()
    )
    ...
    return (
        jsonify(
            {
                "bugs_by_region": {
                    region or "Unknown": count for region, count in bugs_by_region
                },
                "duplicates_by_region": {
                    region or "Unknown": count for region, count in duplicates_by_region
                },
            }
        ),
        200,
...
def get_build_stats():
    ...
    bugs_by_build = (
        db.session.query(Bug.build_version, func.count(Bug.id).label("count"))
        .group_by(Bug.build_version)
        .order_by(func.count(Bug.id).desc())
        .limit(20)
        .all()
    )
    ...
    return (
        jsonify(
            {
                "bugs_by_build": {
                    build or "Unknown": count for build, count in bugs_by_build
                }
            }
        ),
        200,
...
def get_timeline_stats():
    ...
    bugs_per_day = (
        ...
        .filter(Bug.created_at >= start_date)
    ...
    duplicates_per_day = (
        ...
        .filter(DuplicateHistory.detected_at >= start_date)
        .group_by(func.date(DuplicateHistory.detected_at))
        .order_by("date")
        .all()
    )
        ...
        jsonify(
            {
                ...
                "bugs_per_day": [
                    {"date": str(date), "count": count} for date, count in bugs_per_day
                ],
                "duplicates_per_day": [
...
def get_quality_stats():
    ...
    by_reporter = (
        db.session.query(
            LowQualityQueue.reporter, func.count(LowQualityQueue.id).label("count")
        )
        .group_by(LowQualityQueue.reporter)
        .order_by(func.count(LowQualityQueue.id).desc())
        .limit(10)
        .all()
    )
...
def get_performance_stats():
    ...
    false_positive_rate = (
        (promotions / total_duplicates * 100) if total_duplicates > 0 else 0
    )

    # Average match scores
    avg_match_score = (
        db.session.query(func.avg(Bug.match_score))
        .filter(Bug.match_score.isnot(None))
        .scalar()
    )
    ...
    score_ranges = {
        ...
        "0.70-0.85": Bug.query.filter(
            Bug.match_score >= 0.70, Bug.match_score < 0.85
        ).count(),
        ...
        jsonify(
            {
                ...
                "average_match_score": (
                    round(avg_match_score, 3) if avg_match_score else None
                ),
    ...
    return jsonify(
        {
            ...
            "components": {"database": db_status, "vector_store": vector_store_status},
