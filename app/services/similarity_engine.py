...
class SimilarityEngine:
    ...
    def find_similar_bugs(
        self, incoming_bug_data: Dict, threshold: float = 0.85, top_k: int = 10
    ) -> List[Dict]:
        """
        ...
        for bug_id, vector_score in vector_candidates:
            ...
            if (
                bug.status in ["Resolved", "Closed"]
                and bug.classification_tag != "Recurring"
            ):
                continue
            ...
            hybrid_score = self._compute_hybrid_score(
                vector_score, metadata_score, vector_weight=0.7, metadata_weight=0.3
            )
            ...
            if (
                hybrid_score >= threshold * 0.8
            ):  # Lower threshold for initial candidates
                candidates.append(
                    {
                        "bug_id": bug.id,
                        "bug": bug,
                        ...
                        "is_cross_region": self._is_cross_region(
                            incoming_bug_data, bug
                        ),
    ...
    def _build_text_for_matching(self, bug_data: Dict) -> str:
        ...
        parts = [
            ...
            (
                f"Build: {bug_data.get('build_version', '')}"
                if bug_data.get("build_version")
                else ""
            ),
    ...
    def _compute_metadata_similarity(
        self, incoming_data: Dict, existing_bug: Bug
    ) -> float:
        """
        ...
        if incoming_data.get("build_version") and existing_bug.build_version:
            if incoming_data["build_version"] == existing_bug.build_version:
                score += 0.3
            elif self._is_similar_build(
                incoming_data["build_version"], existing_bug.build_version
            ):
                score += 0.15
            total_weight += 0.3
    ...
    def _normalize_cross_region_score(
        self, score: float, penalty: float = 0.05
    ) -> float:
        """
    ...
    def _get_matching_fields(self, incoming_data: Dict, existing_bug: Bug) -> List[str]:
        ...
        for field, existing_value in field_map.items():
            ...
            if (
                incoming_value
                and existing_value
                and str(incoming_value).lower() == str(existing_value).lower()
            ):
                matching.append(field)

        return matching

    def _get_differing_fields(
        self, incoming_data: Dict, existing_bug: Bug
    ) -> List[str]:
