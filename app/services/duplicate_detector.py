...
class DuplicateDetector:
    ...
    def process_incoming_bug(self, bug_data: Dict) -> Tuple[Optional[Bug], str, Dict]:
        ...
        if not is_quality_ok:
            logger.info(
                f"Bug submission from {reporter} failed quality check: {quality_issues}"
            )
    ...
    def _handle_low_quality(
        self, bug_data: Dict, quality_issues: list
    ) -> Tuple[None, str, Dict]:
        """
        ...
        return (
            None,
            "low_quality",
            {"low_quality_id": low_quality_entry.id, "issues": quality_issues},
        )

    def _handle_duplicate(
        self, bug_data: Dict, match: Dict, blocked: bool
        ...
        if blocked:
            ...
            logger.info(
                f"Blocked duplicate submission (parent: {parent_bug.id}, score: {match_score:.3f})"
            )
            ...
            logger.info(
                f"Created duplicate bug {bug.id} (parent: {parent_bug.id}, score: {match_score:.3f})"
            )
    ...
    def check_recurring_pattern(self, bug: Bug) -> bool:
        ...
        if duplicate_count >= 3:
            ...
            logger.info(
                f"Marked bug {bug.id} and parent {bug.parent_bug_id} as Recurring"
            )
            return True
